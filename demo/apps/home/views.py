...
def productos_view(request, pagina):
	...
	paginator = Paginator(lista_prod, 3) #Cuantos productos quieres por paginar
	try:
		page = int(pagina)
	except:
		page = 1
...
def contacto_view(request):
	info_enviado = False #Define si se envio la informacion o no se envio
	email = ""
	titulo = ""
	texto = ""
	if request.method == "POST": # Pregutna si se realizo un post 
		#si se realizo un post se capturan los valores ingresados en el formulario
		...
		if formulario.is_valid():
			...
			# Configuraion para enviar mensaje

			to_admin = 'miguel.isel17@gmail.com'
			html_content = "Informacion recibida ***Mensaje** %s"%(texto)
			msg = EmailMultiAlternatives('Correo de contacto',html_content,'from@server.com',[to_admin])
			msg.attach_alternative(html_content,'text/html')#definimos el contenido html
			msg.send() # Enviamos el correo
			

	else: # si no se muestra un nuevo formulario
...
#vista con paso de parametros
def singleProduct_view(request,id_prod): 
	prod = producto.objects.get(id=id_prod)
	cats = prod.categoria.all() # estamos opteniendo las categorias del producto encontrado
